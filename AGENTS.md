...
| Prompts | `prompts/*.md` | Master prompt, twelve task prompts, motion recipes and the offline conformance prompt |
...
9. When the specification changes, update `prompts/14-offline-prompt.md` in the same commit: it
   mirrors the rule set inline for agents without web access, and it carries the spec version it was
   written against.
10. **Never hand-edit `components/` or `templates/`.** Edit `scripts/lib_data.py` (which components
...
11. Adding a category means editing the registry in `scripts/lib_data.py` and the per-category entries
